...
Traccar.app.getAttributePreference('volumeUnit'))},volumeConverter:function(value){return Ext.getStore('VolumeUnits').convertValue(value,Traccar.app.getAttributePreference('volumeUnit'))},hoursFormatter:function(value){return Ext.getStore('HoursUnits').formatValue(value,'h')},hoursConverter:function(value){return Ext.getStore('HoursUnits').convertValue(value,'h')},durationFormatter:function(value){return Ext.getStore('HoursUnits').formatValue(value,'h',!0)},deviceIdFormatter:function(value){var store;
return 0!==value?(store=Ext.getStore('AllDevices'),0===store.getTotalCount()&&(store=Ext.getStore('Devices')),(value=store.getById(value))?value.get('name'):''):null},groupIdFormatter:function(value){var group;return 0!==value?(group=Ext.getStore('AllGroups'),0===group.getTotalCount()&&(group=Ext.getStore('Groups')),(group=group.getById(value))?group.get('name'):value):null},geofenceIdFormatter:function(value){var store;return 0!==value?(store=Ext.getStore('AllGeofences'),0===store.getTotalCount()&&
(store=Ext.getStore('Geofences')),(value=store.getById(value))?value.get('name'):''):null},calendarIdFormatter:function(value){var store;return 0!==value?(store=Ext.getStore('AllCalendars'),0===store.getTotalCount()&&(store=Ext.getStore('Calendars')),(value=store.getById(value))?value.get('name'):''):null},driverUniqueIdFormatter:function(value){var driver;return 0!==value?(driver=Ext.getStore('AllDrivers'),0===driver.getTotalCount()&&(driver=Ext.getStore('Drivers')),(driver=driver.findRecord('uniqueId',
value,0,!1,!0,!0))?value+' ('+driver.get('name')+')':value):null},maintenanceIdFormatter:function(value){var store;return 0!==value?(store=Ext.getStore('AllMaintenances'),0===store.getTotalCount()&&(store=Ext.getStore('Maintenances')),(value=store.getById(value))?value.get('name'):''):null},lastUpdateFormatter:function(value){var interval;if(value){value=Math.floor((new Date-value)/1E3);0>value&&(value=0);interval=Math.floor(value/86400);if(1<interval)return interval+' '+Strings.sharedDays;interval=
...
combobox.displayField='name';combobox.valueField='key';break;case 'speedUnit':combobox.store='SpeedUnits';combobox.displayField='name';combobox.valueField='key';break;case 'volumeUnit':combobox.store='VolumeUnits';combobox.displayField='fullName';combobox.valueField='key';break;case 'timezone':combobox.store='AllTimezones',combobox.displayField='key'}else combobox.xtype='textfield'}valueField.getXType()!==combobox.xtype||'customNumberField'===combobox.xtype&&valueField.dataType!==combobox.dataType?
(this.getView().down('form').insert(this.getView().down('form').items.indexOf(valueField),combobox),this.getView().down('form').remove(valueField)):'customNumberField'===combobox.xtype?(valueField.setConfig(combobox),valueField.validate()):'combobox'===combobox.xtype&&(valueField.setConfig(combobox),valueField.setValue())}}});
...
Traccar.app.getPreference('deviceReadonly',!1)&&!Traccar.app.getUser().get('administrator');readonly=Traccar.app.getPreference('readonly',!1)&&!Traccar.app.getUser().get('administrator');this.lookupReference('toolbarAddButton').setDisabled(readonly||deviceReadonly);this.lookupReference('toolbarDeviceMenu').setHidden(readonly||deviceReadonly);setInterval(function(){self.getView().getView().refresh()},Traccar.Style.refreshPeriod)},onCommandClick:function(){var deviceId,dialog,typesStore;deviceId=this.getView().getSelectionModel().getSelection()[0].get('id');
...
filter:'boolean'},{text:Strings.notificationNotificators,dataIndex:'notificators',flex:2,filter:{type:'arraylist',idField:'type',labelField:'name',store:'AllNotificators'},renderer:function(value){var result='',notificators;if(value)for(notificators=value.split(/[ ,]+/).filter(Boolean),value=0;value<notificators.length;value++)result+=Traccar.app.getNotificatorString(notificators[value])+(value<notificators.length-1?', ':'');return result}},{text:Strings.sharedCalendar,dataIndex:'calendarId',flex:1,
minWidth:Traccar.Style.columnWidthNormal,hidden:!0,filter:{type:'list',labelField:'name',store:'AllCalendars'},renderer:Traccar.AttributeFormatter.getFormatter('calendarId')}]}});
...
Ext.define('Traccar.view.dialog.ComputedAttribute',{extend:Traccar.view.dialog.BaseEdit,controller:'computedAttribute',title:Strings.sharedComputedAttribute,items:{xtype:'form',listeners:{afterrender:function(view){view=view.up('panel').lookupReference('expressionField');view.setValue(Ext.String.htmlDecode(view.getValue()))}},items:[{xtype:'textfield',name:'description',fieldLabel:Strings.sharedDescription},{xtype:'combobox',name:'attribute',fieldLabel:Strings.sharedAttribute,store:'PositionAttributes',
displayField:'name',valueField:'key',listeners:{change:'onAttributeChange'}},{xtype:'textareafield',reference:'expressionField',name:'expression',fieldLabel:Strings.sharedExpression,allowBlank:!1},{xtype:'combobox',name:'type',reference:'typeComboField',store:'AttributeValueTypes',fieldLabel:Strings.sharedType,displayField:'name',valueField:'id',editable:!1}]},buttons:[{glyph:'xf128@FontAwesome',tooltip:Strings.sharedCheckComputedAttribute,tooltipType:'title',minWidth:0,handler:'onCheckClick'},{glyph:'xf00c@FontAwesome',
...
Ext.define('Traccar.view.dialog.DeviceAccumulatorsController',{extend:Ext.app.ViewController,alias:'controller.deviceAccumulators',onSetClick:function(){var totalDistance,data={deviceId:this.getView().deviceId};totalDistance=this.lookupReference('totalDistance');isNaN(totalDistance.getRawValue())||(data.totalDistance=totalDistance.getValue());totalDistance=this.lookupReference('hours');isNaN(totalDistance.getRawValue())||(data.hours=totalDistance.getValue());Ext.Ajax.request({scope:this,method:'PUT',
url:'api/devices/'+data.deviceId+'/accumulators',jsonData:Ext.util.JSON.encode(data),callback:function(options,success,response){success||Traccar.app.showError(response)}});this.closeView()}});
...
reference:'toolbarEditButton',glyph:'xf040@FontAwesome',tooltip:Strings.sharedEdit},{handler:'onRemoveClick',reference:'toolbarRemoveButton',glyph:'xf00d@FontAwesome',tooltip:Strings.sharedRemove},{handler:'onCommandClick',reference:'deviceCommandButton',glyph:'xf093@FontAwesome',tooltip:Strings.deviceCommand},{xtype:'deviceMenu',reference:'toolbarDeviceMenu',enableToggle:!1}]},listeners:{rowclick:'onSelectionChange',itemkeyup:'onSelectionChange'},viewConfig:{enableTextSelection:!0,getRowClass:function(record){var result=
...
void 0!==this.period&&dialog.lookupReference('periodField').setValue(this.period);dialog.show()},updateButtons:function(){var reportType,disabled,time;reportType=this.lookupReference('reportTypeField').getValue();disabled=this.deviceId&&0!==this.deviceId.length||this.groupId&&0!==this.groupId.length;time=this.fromDate&&this.fromTime&&this.toDate&&this.toTime;disabled=!reportType||!disabled||!time||this.reportProgress;this.lookupReference('showButton').setDisabled(disabled);this.lookupReference('exportButton').setDisabled('chart'===
reportType||disabled);this.lookupReference('emailButton').setDisabled('chart'===reportType||disabled)},onReportClick:function(button){var reportType,from,to;this.getGrid().getSelectionModel().deselectAll();(reportType=this.lookupReference('reportTypeField').getValue())&&(this.deviceId||this.groupId)&&(from=new Date(this.fromDate.getFullYear(),this.fromDate.getMonth(),this.fromDate.getDate(),this.fromTime.getHours(),this.fromTime.getMinutes(),this.fromTime.getSeconds(),this.fromTime.getMilliseconds()),
...
selectDevice:function(device){device&&this.getGrid().getSelectionModel().deselectAll()},selectReport:function(object){var reportType=this.lookupReference('reportTypeField').getValue();if(object instanceof Traccar.model.Position)if('route'===reportType)this.getGrid().getSelectionModel().select([object],!1,!0),this.getGrid().getView().focusRow(object);else if('events'===reportType||'stops'===reportType)object=this.getGrid().getStore().findRecord('positionId',object.get('id'),0,!1,!0,!0),this.getGrid().getSelectionModel().select([object],
!1,!0),this.getGrid().getView().focusRow(object)},deselectFeature:function(){'trips'!==this.lookupReference('reportTypeField').getValue()&&this.getGrid().getSelectionModel().deselectAll()},selectTrip:function(trip){var from,to;from=new Date(trip.get('startTime'));to=new Date(trip.get('endTime'));Ext.getStore('ReportRoute').removeAll();Ext.getStore('ReportRoute').showMarkers=this.showMarkers;Ext.getStore('ReportRoute').load({params:{deviceId:trip.get('deviceId'),from:from.toISOString(),to:to.toISOString()}})},
...
dataIndex:'deviceId',renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},{text:Strings.sharedDistance,dataIndex:'distance',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.reportStartOdometer,dataIndex:'startOdometer',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.reportEndOdometer,dataIndex:'endOdometer',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.reportAverageSpeed,dataIndex:'averageSpeed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},
{text:Strings.reportMaximumSpeed,dataIndex:'maxSpeed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},{text:Strings.reportEngineHours,dataIndex:'engineHours',renderer:Traccar.AttributeFormatter.getFormatter('duration')},{text:Strings.reportSpentFuel,dataIndex:'spentFuel',renderer:Traccar.AttributeFormatter.getFormatter('spentFuel')}],tripsColumns:[{text:Strings.reportDeviceName,dataIndex:'deviceId',renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},{text:Strings.reportStartTime,
dataIndex:'startTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('startTime')},{text:Strings.reportStartOdometer,dataIndex:'startOdometer',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.reportStartAddress,dataIndex:'startAddress',renderer:Traccar.AttributeFormatter.getFormatter('address')},{text:Strings.reportEndTime,dataIndex:'endTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('endTime')},{text:Strings.reportEndOdometer,
...
dataIndex:'startTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('startTime')},{text:Strings.positionOdometer,dataIndex:'startOdometer',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.positionAddress,dataIndex:'address',renderer:Traccar.AttributeFormatter.getFormatter('address')},{text:Strings.reportEndTime,dataIndex:'endTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('endTime')},{text:Strings.reportDuration,dataIndex:'duration',
renderer:Traccar.AttributeFormatter.getFormatter('duration')},{text:Strings.reportEngineHours,dataIndex:'engineHours',renderer:Traccar.AttributeFormatter.getFormatter('duration')},{text:Strings.reportSpentFuel,dataIndex:'spentFuel',renderer:Traccar.AttributeFormatter.getFormatter('spentFuel')}]});
...
Ext.define('Traccar.view.map.BaseMap',{extend:Ext.panel.Panel,xtype:'baseMapView',layout:'fit',getMap:function(){return this.map},getMapView:function(){return this.mapView},initMap:function(){var server,type,bingKey,zoom,maxZoom,target,self=this;server=Traccar.app.getServer();type=Traccar.app.getPreference('map',null);bingKey=server.get('bingKey');switch(type){case 'custom':server=new ol.layer.Tile({source:new ol.source.XYZ({url:Ext.String.htmlDecode(server.get('mapUrl')),attributions:''})});break;
case 'bingRoad':server=new ol.layer.Tile({source:new ol.source.BingMaps({key:bingKey,imagerySet:'Road'})});break;case 'bingAerial':server=new ol.layer.Tile({source:new ol.source.BingMaps({key:bingKey,imagerySet:'Aerial'})});break;case 'bingHybrid':server=new ol.layer.Tile({source:new ol.source.BingMaps({key:bingKey,imagerySet:'AerialWithLabels'})});break;case 'carto':server=new ol.layer.Tile({source:new ol.source.XYZ({url:'https://cartodb-basemaps-{a-d}.global.ssl.fastly.net/light_all/{z}/{x}/{y}.png',
...
Traccar.app.getPreference('latitude',Traccar.Style.mapDefaultLat);bingKey=Traccar.app.getPreference('longitude',Traccar.Style.mapDefaultLon);zoom=Traccar.app.getPreference('zoom',Traccar.Style.mapDefaultZoom);maxZoom=Traccar.app.getAttributePreference('web.maxZoom',Traccar.Style.mapMaxZoom);this.mapView=new ol.View({center:ol.proj.fromLonLat([bingKey,type]),zoom:zoom,maxZoom:maxZoom});this.map=new ol.Map({target:this.body.dom.id,layers:[server],view:this.mapView});(server=Traccar.app.getPreference('poiLayer',
...
device),this.updateLiveRoute(position,device)},updateAccuracy:function(position,device){var center,radius,feature,projection;radius=this.getView().getMapView();feature=this.accuracyCircles[position.get('deviceId')];position.get('accuracy')?(projection=radius.getProjection(),center=ol.proj.fromLonLat([position.get('longitude'),position.get('latitude')]),projection=ol.proj.getPointResolution(projection,radius.getResolution(),center),radius=position.get('accuracy')/ol.proj.Units.METERS_PER_UNIT.m*radius.getResolution()/
...
null!==wkt))){geometry=mapView.getProjection();for(mapView=0;mapView<wkt.length;mapView+=2)lat=Number(wkt[mapView]),lon=Number(wkt[mapView+1]),points.push(ol.proj.transform([lon,lat],'EPSG:4326',geometry));geometry=new ol.geom.LineString(points)}return geometry},geometryToWkt:function(projection,geometry){var result;if(geometry instanceof ol.geom.Circle)result=geometry.getCenter(),geometry=geometry.getRadius(),geometry=[result[0]+geometry,result[1]],result=ol.proj.transform(result,projection,'EPSG:4326'),
...
Ext.define('Traccar.view.dialog.Server',{extend:Traccar.view.dialog.BaseEdit,controller:'mapPicker',title:Strings.serverTitle,items:{xtype:'form',listeners:{afterrender:function(view){view=view.up('panel').lookupReference('mapUrlField');view.setValue(Ext.String.htmlDecode(view.getValue()))}},items:[{xtype:'fieldset',title:Strings.sharedPreferences,items:[{xtype:'clearableComboBox',name:'map',fieldLabel:Strings.mapLayer,store:'MapTypes',displayField:'name',valueField:'key'},{xtype:'textfield',name:'bingKey',
fieldLabel:Strings.mapBingKey},{xtype:'textfield',reference:'mapUrlField',name:'mapUrl',fieldLabel:Strings.mapCustom},{xtype:'numberfield',reference:'latitude',name:'latitude',fieldLabel:Strings.positionLatitude,decimalPrecision:Traccar.Style.coordinatePrecision},{xtype:'numberfield',reference:'longitude',name:'longitude',fieldLabel:Strings.positionLongitude,decimalPrecision:Traccar.Style.coordinatePrecision},{xtype:'numberfield',reference:'zoom',name:'zoom',fieldLabel:Strings.serverZoom},{xtype:'checkboxfield',
...
onEditClick:function(){var dialog,user=this.getView().getSelectionModel().getSelection()[0];dialog=Ext.create('Traccar.view.dialog.User',{selfEdit:user.get('id')===Traccar.app.getUser().get('id')});dialog.down('form').loadRecord(user);dialog.show()},onAddClick:function(){var user,dialog;user=Ext.create('Traccar.model.User');Traccar.app.getUser().get('administrator')&&user.set('deviceLimit',-1);Traccar.app.getUser().get('expirationTime')&&user.set('expirationTime',Traccar.app.getUser().get('expirationTime'));
...
Ext.define('Traccar.view.dialog.NotificationController',{extend:Traccar.view.dialog.BaseEditController,alias:'controller.notification',init:function(){this.lookupReference('calendarCombo').setHidden(Traccar.app.getBooleanAttributePreference('ui.disableCalendars'))},onTypeChange:function(view,value){this.lookupReference('alarmsField').setHidden('alarm'!==value)},onAlarmsLoad:function(view){var attributes;attributes=view.up('form').getRecord().get('attributes')||{};attributes.alarms&&(view.suspendEvents(!1),
view.setValue(attributes.alarms.split(',')),view.resumeEvents())},onAlarmsChange:function(view,value){var record=view.up('window').down('form').getRecord();view=record.get('attributes')||{};value=value.join();view.alarms!==value&&(view.alarms=value,record.set('attributes',view),record.dirty=!0)}});
...
{text:Strings.notificationAlways,dataIndex:'always',renderer:Traccar.AttributeFormatter.getFormatter('always'),filter:'boolean'},{text:Strings.sharedAlarms,dataIndex:'attributes',renderer:function(value){var key,result='',alarms=value&&value.alarms;if(alarms)for(alarms=alarms.split(','),value=0;value<alarms.length;value++)key='alarm'+alarms[value].charAt(0).toUpperCase()+alarms[value].slice(1),result&&(result+=', '),result+=Strings[key]||key;return result}},{text:Strings.notificationNotificators,
...
listeners:{change:'onNameChange'}},{xtype:'customNumberField',name:'start',reference:'startField',fieldLabel:Strings.maintenanceStart},{xtype:'customNumberField',name:'period',reference:'periodField',allowBlank:!1,fieldLabel:Strings.maintenancePeriod,validator:function(value){return 0!==this.parseValue(value)?!0:Strings.errorZero}}]}]}});
...
Ext.define('Traccar.view.SettingsMenuController',{extend:Ext.app.ViewController,alias:'controller.settings',init:function(){var admin,manager,readonly;admin=Traccar.app.getUser().get('administrator');manager=0!==Traccar.app.getUser().get('userLimit');readonly=Traccar.app.getPreference('readonly',!1);admin&&(this.lookupReference('settingsServerButton').setHidden(!1),this.lookupReference('settingsStatisticsButton').setHidden(!1),this.lookupReference('settingsComputedAttributesButton').setHidden(Traccar.app.getBooleanAttributePreference('ui.disableComputedAttributes')));
...
Ext.define('Traccar.controller.Root',{extend:Ext.app.Controller,alias:'controller.root',init:function(){var i,data,attribute,chartTypesStore,maintenanceTypesStore;chartTypesStore=Ext.getStore('ReportChartTypes');maintenanceTypesStore=Ext.getStore('MaintenanceTypes');Ext.state.Manager.setProvider(new Ext.state.CookieProvider);data=Ext.getStore('PositionAttributes').getData().items;for(i=0;i<data.length;i++)attribute=data[i],Traccar.model.Position.addFields([{name:'attribute.'+attribute.get('key'),
attributeKey:attribute.get('key'),calculate:this.calculateAttribute,persist:!1}]),'number'===attribute.get('valueType')&&(chartTypesStore.add({key:'attribute.'+attribute.get('key'),name:attribute.get('name')}),maintenanceTypesStore.add(attribute))},calculateAttribute:function(data){data=data.attributes[this.attributeKey];return void 0!==data?Traccar.AttributeFormatter.getAttributeConverter(this.attributeKey)(data):data},onLaunch:function(){Ext.Ajax.request({scope:this,url:'api/server',callback:this.onServerReturn})},
onServerReturn:function(options,success,response){options={};if(success){Traccar.app.setServer(Ext.decode(response.responseText));if(success=Ext.Object.fromQueryString(window.location.search).token)options.token=success;Ext.Ajax.request({scope:this,url:'api/session',method:'GET',params:options,callback:this.onSessionReturn})}else Traccar.app.showError(response)},onSessionReturn:function(options,success,response){Ext.get('spinner').setVisible(!1);success?(Traccar.app.setUser(Ext.decode(response.responseText)),
this.loadApp()):(this.login=Ext.create('widget.login',{listeners:{scope:this,login:this.onLogin}}),this.login.show())},onLogin:function(){this.login.close();this.loadApp()},loadApp:function(){var attribution;window.webkit&&window.webkit.messageHandlers.appInterface&&window.webkit.messageHandlers.appInterface.postMessage('login');window.appInterface&&window.appInterface.postMessage('login');Ext.getStore('Groups').load();Ext.getStore('Drivers').load();Ext.getStore('Geofences').load();Ext.getStore('Calendars').load();
...
Ext.Object.fromQueryString(window.location.search);delete params[param];Ext.Object.isEmpty(params)?window.history.pushState(null,null,window.location.pathname):window.history.pushState(null,null,window.location.pathname+'?'+Ext.Object.toQueryString(params))},asyncUpdate:function(first){var self=this,protocol,pathname;protocol='https:'===window.location.protocol?'wss:':'ws:';pathname=window.location.pathname.substring(0,window.location.pathname.lastIndexOf('/')+1);protocol=new WebSocket(protocol+'//'+
window.location.host+pathname+'api/socket');protocol.onclose=function(){Traccar.app.showToast(Strings.errorSocket,Strings.errorTitle);Ext.Ajax.request({url:'api/devices',success:function(response){self.updateDevices(Ext.decode(response.responseText))},failure:function(response){401===response.status&&window.location.reload()}});Ext.Ajax.request({url:'api/positions',headers:{Accept:'application/json'},success:function(response){self.updatePositions(Ext.decode(response.responseText))}});setTimeout(function(){self.asyncUpdate(!1)},
Traccar.Style.reconnectTimeout)};protocol.onmessage=function(event){event=Ext.decode(event.data);event.devices&&self.updateDevices(event.devices);event.positions&&(self.updatePositions(event.positions,first),first=!1);event.events&&self.updateEvents(event.events)}},updateDevices:function(array){var i,store,entity;store=Ext.getStore('Devices');for(i=0;i<array.length;i++)(entity=store.getById(array[i].id))&&entity.set({status:array[i].status,lastUpdate:array[i].lastUpdate,geofenceIds:array[i].geofenceIds},
